...
// every other page) - only the page CONTENT below the "Overview" title
// uses this page's own dark, self-contained styling. Every selector in
// the CSS below is scoped under .dp-root specifically because global.css
// already defines a few of the same generic names (.grid, for one) -
// unscoped selectors here would have silently changed those on every
...
function Dot({ s, pulse }) {
    return (
        <span className="dp-dot" style={{ background: S[s], boxShadow: `0 0 0 3px ${S[s]}22` }}>
                            ...
                            <div className="dp-blockers">
                                ...
                                {blockers.map((b, i) => (
                                    <div key={i} className="dp-blocker">
                                        ...
                                        <span className="dp-bicon" style={{ color: S[b.sev], background: `${S[b.sev]}18` }}>
                                            <b.Icon size={14} />
                                        </span>
...
const CSS = `
.dp-root{
  --bg:#0b0e13; --panel:#141a23; --panel2:#1a222e; --line:#232d3b;
  --line2:#2c3948; --text:#e6edf5; --muted:#8b98ab; --faint:#5a6675;
  --teal:${TEAL};
  font-family:'Space Grotesk',system-ui,sans-serif;
  background:
    radial-gradient(900px 500px at 82% -8%, #16303050, transparent),
    var(--bg);
...
.dp-root :focus-visible{outline:2px solid var(--teal); outline-offset:2px; border-radius:6px;}
...
.dp-env-pills{display:flex; gap:3px; background:var(--panel); padding:3px; border-radius:9px; border:1px solid var(--line); width:fit-content; margin-bottom:14px; flex-wrap:wrap;}
.dp-env-pill{border:0; background:transparent; color:var(--muted); font-size:12.5px; font-weight:500;
  padding:5px 11px; border-radius:7px; transition:.15s;}
.dp-env-pill:hover{color:var(--text);}
.dp-env-pill.on{background:var(--panel2); color:var(--text); box-shadow:inset 0 0 0 1px var(--line2);}
...
.dp-ribbon{display:flex; align-items:center; gap:26px; flex-wrap:wrap;
  ...
  background:linear-gradient(180deg,#161d27,#121821); margin-bottom:16px; position:relative; overflow:hidden;}
...
.dp-verdict h1{margin:0; font-size:24px; font-weight:600; letter-spacing:-.02em;}
.dp-verdict p{margin:3px 0 0; font-size:13px; color:var(--muted);}
.dp-vitals{display:flex; gap:12px; margin-left:auto; flex-wrap:wrap;}
.dp-vital{background:#0e141c; border:1px solid var(--line); border-radius:12px; padding:11px 15px; min-width:118px;}
.dp-vital.dp-wide{min-width:230px;}
.dp-vlabel{display:flex; align-items:center; gap:6px; font-size:11.5px; color:var(--faint); font-weight:500;}
.dp-vval{display:flex; align-items:baseline; gap:2px; font-size:26px; font-weight:600; margin-top:6px; letter-spacing:-.02em;}
.dp-vslash{font-size:15px; color:var(--faint); font-weight:500;}
.dp-vsub{font-size:11px; color:var(--muted); display:block; margin-top:4px;}
...
.dp-spark-col .good{background:${S.ok}88; border-radius:1px; min-height:1px;}
...
.dp-panel{background:var(--panel); border:1px solid var(--line); border-radius:16px; overflow:hidden; margin-bottom:16px;}
.dp-grid .dp-panel, .dp-rail-col .dp-panel{margin-bottom:0;}
.dp-panel-head{display:flex; align-items:center; justify-content:space-between; padding:14px 16px; border-bottom:1px solid var(--line); flex-wrap:wrap; gap:8px;}
.dp-panel-title{display:flex; align-items:center; gap:9px; font-size:14px; font-weight:600; letter-spacing:-.01em;}
.dp-panel-title svg{color:var(--muted);}
.dp-count-pill{background:#0e141c; border:1px solid var(--line2); color:var(--muted); font-size:12px;
  min-width:22px; text-align:center; padding:2px 7px; border-radius:20px; font-weight:600;}

.dp-seg{display:flex; gap:2px; background:#0e141c; padding:3px; border-radius:8px; border:1px solid var(--line);}
.dp-seg-b{border:0; background:transparent; color:var(--muted); font-size:12px; font-weight:500; padding:4px 10px; border-radius:6px; transition:.15s;}
.dp-seg-b.on{background:var(--panel2); color:var(--text);}
...
.dp-board-head{padding:9px 16px; font-size:11px; color:var(--faint); border-bottom:1px solid var(--line); font-weight:500;}
.dp-hcell{text-align:right;}
.dp-rows{display:flex; flex-direction:column; max-height:520px; overflow-y:auto;}
.dp-row{position:relative; padding:12px 16px 12px 18px; border-bottom:1px solid #1a212c; transition:background .12s;}
.dp-row:last-child{border-bottom:0;}
.dp-row:hover{background:#161d27;}
...
.dp-wf-name{font-size:13px; font-weight:500; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;}
.dp-run-id{display:flex; align-items:center; gap:8px; font-size:11px; color:var(--faint); white-space:nowrap; overflow:hidden; text-overflow:ellipsis;}
.dp-branch{display:flex; align-items:center; gap:7px; color:var(--muted); font-size:12.5px; min-width:0;}
.dp-branch svg{color:var(--faint); flex:0 0 auto;}
.dp-branch .dp-mono{white-space:nowrap; overflow:hidden; text-overflow:ellipsis;}
.dp-sha{font-size:12px; color:var(--muted);}
.dp-muted{color:var(--faint);}
.dp-who{display:inline-grid; place-items:center; width:26px; height:26px; border-radius:7px; font-size:11px;
  font-weight:600; background:#20303f; color:#bcd; border:1px solid var(--line2); margin-left:auto;}
.dp-time{display:flex; align-items:center; gap:5px; justify-content:flex-end; font-size:12px; color:var(--muted);}
.dp-time svg{color:var(--faint);}
.dp-progress{grid-column:1/-1; height:2px; background:#0e141c; border-radius:2px; margin-top:11px; overflow:hidden;}
...
.dp-empty{padding:34px 16px; text-align:center; color:var(--faint); font-size:13px;}
.dp-empty.dp-small{padding:20px;}

.dp-blockers{display:flex; flex-direction:column;}
.dp-blocker{position:relative; display:flex; align-items:flex-start; gap:11px; padding:13px 15px 13px 17px; border-bottom:1px solid #1a212c;}
...
.dp-btitle{font-size:12.5px; font-weight:600;}
.dp-bsub{font-size:11.5px; color:var(--muted); line-height:1.4;}
.dp-baction{display:flex; align-items:center; gap:1px; background:transparent; border:1px solid var(--line2);
  color:var(--muted); font-size:11.5px; font-weight:500; padding:5px 8px 5px 10px; border-radius:8px;
  white-space:nowrap; align-self:center; transition:.15s;}
.dp-baction:hover{border-color:var(--teal); color:var(--teal);}

.dp-approvals{display:flex; flex-direction:column;}
.dp-approval{display:flex; align-items:center; gap:12px; padding:14px 15px; border-bottom:1px solid #1a212c; flex-wrap:wrap;}
...
.dp-atitle{font-size:13px; font-weight:600;}
.dp-ameta{font-size:11px; color:var(--faint); line-height:1.4;}
...
.dp-abtns .deny{background:transparent; border-color:var(--line2); color:var(--muted);}
.dp-abtns .deny:hover{border-color:${S.down}; color:${S.down};}
.dp-abtns .approve{background:${TEAL}; border-color:${TEAL}; color:#04211f;}
...
.dp-decided.approve{background:${S.ok}1e; color:${S.ok};}
.dp-decided.deny{background:${S.down}1e; color:${S.down};}

.dp-fleet-legend{display:flex; gap:14px; font-size:11.5px; color:var(--muted); flex-wrap:wrap;}
...
.dp-fg-count{font-size:11px; color:var(--faint);}
.dp-fg-count.has-bad{color:${S.down};}
.dp-tiles{display:flex; flex-direction:column; gap:4px;}
.dp-tile{display:flex; align-items:center; gap:9px; padding:7px 10px; border-radius:8px; background:#0e141c; border:1px solid var(--line);
  width:100%; text-align:left; font:inherit; color:inherit;}
.dp-tile.down{background:${S.down}12; border-color:${S.down}44;}
.dp-tile.off{opacity:.6;}
.dp-tname{font-size:12.5px; font-weight:500;}
.dp-tlat{margin-left:auto; font-size:11px; color:var(--faint);}

.dp-envs{display:grid; grid-template-columns:repeat(auto-fill,minmax(240px,1fr)); gap:12px; padding:14px;}
.dp-envcard{background:#0e141c; border:1px solid var(--line); border-radius:12px; padding:15px; transition:.15s; cursor:pointer;}
.dp-envcard:hover{border-color:var(--teal);}
.dp-envcard-top{display:flex; align-items:flex-start; gap:11px; margin-bottom:14px;}
.dp-envcard-top .dp-dot{margin-top:5px;}
.dp-envname{font-size:13px; font-weight:600; display:block;}
.dp-envtarget{display:block; font-size:11.5px; color:var(--teal); margin-top:3px;}
.dp-envfoot{display:flex; align-items:center; justify-content:space-between; font-size:11.5px; color:var(--muted); border-top:1px solid var(--line); padding-top:11px; flex-wrap:wrap; gap:6px;}
.dp-envfoot b{color:var(--text); font-weight:600;}
.dp-envver{display:flex; align-items:center; gap:5px; color:var(--faint);}
a.dp-envver:hover{color:var(--teal);}

.dp-footer{display:flex; justify-content:space-between; align-items:center; padding:4px 6px; font-size:11.5px; color:var(--faint); flex-wrap:wrap; gap:6px;}
