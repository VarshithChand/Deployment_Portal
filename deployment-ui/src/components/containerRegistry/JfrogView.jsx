...
import { getJfrogRepositories, getJfrogImages, getJfrogTags } from "../../services/containerRegistryService";
...
export default function JfrogView() {
    ...
    function refresh() {
        ...
        getJfrogRepositories().then((data) => {
    ...
    function openRepo(repo) {
        ...
        getJfrogImages(repo.key).then((data) => {
            setImages(data);
    ...
    function openImage(image) {
        ...
        getJfrogTags(selectedRepo.key, image.name).then((data) => {
            setTags(data);
    ...
    const filteredRepos = useMemo(() => {
        ...
        return trimmed ? items.filter((r) => r.key.toLowerCase().includes(trimmed)) : items;
        ...
        return (
            ...
            <div className="card">
                ...
                {tagsLoading ? (
                    ...
                    <p className="error-message">{tags?.error || "JFrog is not configured."}</p>
    ...
    if (selectedRepo) {
        ...
        return (
            ...
            <div className="card">
                ...
                <div className="button-row" style={{ justifyContent: "space-between", marginBottom: "12px" }}>
                    <h2 className="card-title" style={{ marginBottom: 0 }}>{selectedRepo.key}</h2>
                ...
                {imagesLoading ? (
                    ...
                    <p className="error-message">{images?.error || "JFrog is not configured."}</p>
    ...
    if (loading) {
        return <div className="card"><p className="empty-state">Loading JFrog repositories...</p></div>;
    }
        ...
        return (
            <div className="card">
                <p className="empty-state" style={{ textAlign: "left" }}>
                    ...
                    <button type="button" className="btn-link" style={{ padding: 0 }} onClick={() => setTab("settings")}>Settings → Credentials → JFrog</button>
                    {" "}to browse this.
                </p>
            </div>
        );
    ...
    return (
        ...
        <div className="card">
            ...
            <div className="button-row" style={{ justifyContent: "space-between", marginBottom: "12px" }}>
                <h2 className="card-title" style={{ marginBottom: 0 }}>JFrog Docker Repositories</h2>
                    ...
                    <table className="table">
                        ...
                        <tbody>
                            ...
                            {pageItems.map((repo) => (
                                ...
                                <tr key={repo.key} className="table-row-clickable" onClick={() => openRepo(repo)}>
                                    <td>{repo.key}</td>
